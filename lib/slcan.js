 ...
 * @param {Integer} baudRate UART baudRate, typically 115200
 * @param {Function} recvFrameCallback Callback function to call when frame is
 * received
 */
function slcan(dev_str, speed, baudRate, recvFrameCallback) {
...
slcan.prototype.open = function() {
    ...
    this._conn = new SerialPort(this._dev_str,{ 'baudRate': this._baudRate });
 ...
 * @method close
 */
slcan.prototype.close = function() {
    this._serialWrite(CMD_CLOSE_BUS);
    this._conn.flush()
    this._conn.close(function (err) {
        console.log('Closing serial port. Error=', err);
    })
}
